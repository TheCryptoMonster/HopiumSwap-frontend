...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://www.hopiumswap.com - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/www.hopiumswap.com)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://www.hopiumswap.com'}
          color={text === 'https://www.hopiumswap.com' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
